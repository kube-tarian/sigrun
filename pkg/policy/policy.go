...
func AddRepo(cpol *kyvernoV1.ClusterPolicy, guid, path string, conf *config.Config) (*kyvernoV1.ClusterPolicy, error) {
	...
	if guidToRepoMeta[guid] != nil {
		return nil, fmt.Errorf("sigrun repo with guid " + guid + " has already been added")
	}

	if g := pubKToGUID[conf.PublicKey]; g != "" {
		return nil, fmt.Errorf("sigrun repo with guid " + guid + " has the same public key as a sigrun repo that has already been added with guid " + g)
	}
